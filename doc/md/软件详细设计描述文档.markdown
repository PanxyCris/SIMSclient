...
|**AccountController.find**|语法|public AccoutPOList find(String message)|
||前置条件|已创建一个AccountBL对象，并且输入符合规则|
||后置条件|调用AccountBL领域对象的check方法|
|**AccountController.newBuild**|语法|public ResultMessage newBuild(AccountPO accountPO)|
||前置条件|已创建一个AccountBL对象，并且输入符合规则|
||后置条|调用AccountBL领域对象的newBuild方法|
|**AccountController.delete**|语法|public ResultMessage delete(AccountPOList accountPOList)|
||前置条件|已创建一个AccountBL对象，并且输入符合规则|
||后置条件|调用AccountBL领域对象的delete方法|
|**AccountController.modify**|语法|public ResultMessage modify(AccountPO accountPO)|
||前置条件|已创建一个AccountBL对象，并且输入符合规则|
...
|AccountBL.newBuild(AccountPO accountPO)|添加账户|
|AccountBL.delete(AccountPO accountPO)|删除账户|
|AccountBL.modify(AccountPO accountPO)|修改账户|
<br/>
　　　AccountBL的接口规范
...
|**AccountBL.find**|语法|public AccoutPOList find(String message)|
||前置条件|已输入正确的搜素关键词|
||后置条件|返回查找到的账户列表|
|**AccountBL.newBuild**|语法|public ResultMessage newBuild(AccountPO accountPO)|
||前置条件|已输入正确的账户格式|
||后置条件|持久化更新数据并返回消息|
|**AccountBL.delete**|语法|public ResultMessage delete(AccountPO accountPO)|
||前置条件|存在选中的待删除的银行账户|
||后置条件|持久化更新数据并返回消息|
|**AccountBL.modify**|语法|public ResultMessage modify(AccountPO accountPO)|
...
|AccountDataService.newBuild(AccountPO accountPO)|新建单一持久化对象|
|AccountDataService.delete(AccountPO accountPO)|删除单一持久化对象|
|AccountDataService.modify(AccountPO accountPO)|更新单一持久化对象|
...
|提供的服务|供接口||
| --- | --- | --- |
|**SKDController.newBuild**|语法|public ResultMessage newBuild(SKDPO skdPO)|
||前置条件|已经创建一个SKDBL领域对象|
||后置条件|调用SKDBL领域对象的newBuild方法|
|**SKDController.modify**|语法|public ResultMessage modify(SKDPO skdPO)|
||前置条件|已经创建一个SKDBL领域对象|
||后置条件|调用SKDBL领域对象的modify方法|
|**SKDController.delete**|语法|public ResultMessage delete(SKDPO skdPO)|
||前置条件|已经创建一个SKDBL领域对象|
||后置条件|调用SKDBL领域对象的delete方法|
|**SKDController.find**|语法|public SKDPOList find(String message)|
||前置条件|已经创建一个SKDBL领域对象|
||后置条件|调用SKDBL领域对象的find方法|
|**SKDController.commit**|语法|public ResultMessage commit(SKDPO skdPO)|
||前置条件|已经创建一个SKDBL领域对象|
...
|SKDBL.newBuild(SKDPO skdPO)|添加单据|
|SKDBL.delete(SKDPO skdPO)|删除单据|
|SKDBL.modify(SKDPO skdPO)|修改单据|
|SKDBL.commit(SKDPO skdPO)|提交单据|
<br/>
　　　　SKDBL的接口规范

|提供的服务|供接口||
| --- | --- | --- |
|**SKDBL.newBuild**|语法|public ResultMessage newBuild(SKDPO skdPO)|
||前置条件|输入正确的收款单单据内容|
||后置条件|新建一个收款单单据并返回消息|
|**SKDBL.modify**|语法|public ResultMessage modify(SKDPO skdPO)|
||前置条件|选中一个收款单单据且输入内容正确|
||后置条件|持久化更新数据并返回消息|
|**SKDBL.delete**|语法|public ResultMessage delete(SKDPO skdPO)|
||前置条件|存在选中的待删除单据|
||后置条件|删除所选中的单据并返回消息|
|**SKDBL.find**|语法|public SKDPOList find(String message)|
||前置条件|已输入正确的搜索关键词|
||后置条件|返回查找到的单据列表|
|**SKDBL.commit**|语法|public ResultMessage commit(SKDPO skdPO)|
||前置条件|选中待提交的单据|
...
|SKDDataService.delete(SKDPO skdPO)|删除单一持久化对象|
|SKDDataService.modify(SKDPO skdPO)|更新单一持久化对象|
|SKDDataService.commit(SKDPO skdPO)|更新单一持久化对象|
...
|提供的服务|供接口||
| --- | --- | --- |
|**XJFYDController.newBuild**|语法|public ResultMessage newBuild(XJFYDPO xjfydPO)|
||前置条件|已经创建一个XJFYDBL领域对象|
||后置条件|调用XJFYDBL领域对象的newBuild方法|
|**XJFYDController.modify**|语法|public ResultMessage modify(XJFYDPO xjfydPO)|
||前置条件|已经创建一个XJFYDBL领域对象|
||后置条件|调用XJFYDBL领域对象的modify方法|
|**XJFYDController.delete**|语法|public ResultMessage delete(XJFYDPO xjfydPO)|
||前置条件|已经创建一个XJFYDBL领域对象|
||后置条件|调用XJFYDBL领域对象的delete方法|
|**XJFYDController.find**|语法|public XJFYDPOList find(String message)|
||前置条件|已经创建一个XJFYDBL领域对象|
||后置条件|调用XJFYDBL领域对象的find方法|
|**XJFYDController.commit**|语法|public ResultMessage commit(XJFYDPO xjfydPO)|
||前置条件|已经创建一个XJFYDBL领域对象|
...
|XJFYDBL.newBuild(XJFYDPO xjfydPO)|添加单据|
|XJFYDBL.delete(XJFYDPO xjfydPO)|删除单据|
|XJFYDBL.modify(XJFYDPO xjfydPO)|修改单据|
|XJFYDBL.commit(XJFYDPO xjfydPO)|提交单据|
<br/>
　　　　XJFYDBL的接口规范

|提供的服务|供接口||
| --- | --- | --- |
|**XJFYDBL.newBuild**|语法|public ResultMessage newBuild(XJFYDPO xjfydPO)|
||前置条件|输入正确的收款单单据内容|
||后置条件|新建一个收款单单据并返回消息|
|**XJFYDBL.modify**|语法|public ResultMessage modify(XJFYDPO xjfydPO)|
||前置条件|选中一个收款单单据且输入内容正确|
||后置条件|持久化更新数据并返回消息|
|**XJFYDBL.delete**|语法|public ResultMessage delete(XJFYDPO xjfydPO)|
||前置条件|存在选中的待删除单据|
||后置条件|删除所选中的单据并返回消息|
|**XJFYDBL.find**|语法|public XJFYDPOList find(String message)|
||前置条件|已输入正确的搜索关键词|
||后置条件|返回查找到的单据列表|
|**XJFYDBL.commit**|语法|public ResultMessage commit(XJFYDPO xjfydPO)|
||前置条件|选中待提交的单据|
||后置条件|将单据提交至总经理，修改单据状态属性并返回消息|
<br/>
...
|XJFYDDataService.delete(XJFYDPO xjfydPO)|删除单一持久化对象|
|XJFYDDataService.modify(XJFYDPO xjfydPO)|更新单一持久化对象|
|XJFYDDataService.commit(XJFYDPO xjfydPO)|更新单一持久化对象|
...
|提供的服务|供接口||
| --- | --- | --- |
|**SetUpAccountController.newBuild**|语法|public ResultMessage newBuild(AccountBookPO accountBookPO)|
||前置条件|已创建一个SetUpAccountBL对象，并且输入符合规则|
||后置条件|调用AccountBL领域对象的newBuild方法|
|**SetUpAccountController.delete**|语法|public ResultMessage delete(AccountBookPO accountBookPO)|
...
| --- | --- |
|服务名|服务|
|SetUpAccountBL.newBuild(AccountBookPO AccountBookPO)|添加账簿|
|SetUpAccountBL.delete(AccountBookPO accountBookPO)|删除账簿|
...
|SetUpAccountDataService.newBuild(AccountBookPO accountBookPO)|新建单一持久化对象|
|SetUpAccountDataService.delete(AccountBookPO accountBookPO)|删除单一持久化对象|
...
<img src="../img/软件详细设图/setupaccount_delete顺序图.png">

<img src="../img/软件详细设图/setupaccount_find顺序图.png">
...
<tr>
<th>CommodityDataService.insertClass(ClassificationPO po)</th>
<th>插入单一持久化对象</th>
</tr>
<tr>
<th>CommodityDataService.deleteClass(ClassificationPO po)</th>
<th>删除单一持久化对象</th>
</tr>
<tr>
<th>CommodityDataService.updateClass(ClassificationPO po)</th>
<th>更新单一持久化对象</th>
</tr>
<tr>
...
该图所示的状态图描述了Sales对象的生存期间的状态序列、引起转移的事件，以及因状态转移而伴随的动作。
...
| **模块** | **职责** |
| -------| ------- |
| PurchaseController | 负责实现制定进货单界面所需要的服务 |
...
该图所示的状态图描述了Purchase对象的生存期间的状态序列、引起转移的事件，以及因状态转移而伴随的动作。
