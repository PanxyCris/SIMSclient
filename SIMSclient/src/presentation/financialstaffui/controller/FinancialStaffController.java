...
public class FinancialStaffController {
	...
	public void mainPage() throws Exception{
		changeStage(mainID,user);

    }

	@FXML
	public void fresh() throws Exception{
		startUI(current,user);
	}
	...
	public void accountManage() throws Exception{
		changeStage("AccountManageUI",user);
	}
	@FXML
	public void makeReceipt() throws Exception{
		changeStage("MakeReceiptUI",user);
	}
	@FXML
	public void viewTable() throws Exception{
		changeStage("ViewTableUI",user);
	}
	@FXML
	public void initAccount() throws Exception{
		changeStage("InitAccountUI",user);
	}
	...
	public void changeStage(String currentID,UserVO user) throws Exception{

   	    startUI(currentID,user);
	    previous = current;
	    current = currentID;
	    stack.push(current);
	}

	public void startUI(String currentID,UserVO user){
	    Stage stage = (Stage) pane.getScene().getWindow();
	    stage.close();
			Platform.runLater(new Runnable() {
            public void run() {
                try {
                   switch(currentID){
   		            ...
   		            case "PaymentMakeBillUI":new PaymentMakeBillUI().start(user);break;
   		            case "ReceiveMakeBillUI":new ReceiveMakeBillUI().start(user);break;
